...
def main():
    ...
    scara = manipulator.Control('ABB IRB 910SC (SCARA)', manipulator.DH_parameters(
    ...
    if tP_smooth == True:
        ...
        try:
            ...
            [smooth_trajectory[0], smooth_trajectory[1], smooth_trajectory[2]
             ] = scara.smooth_trajectory(trajectory_str)
    ...
    if True in tP_err[0]:
        ...
        animator = animation.FuncAnimation(scara.figure, scara.start_animation, init_func=scara.init_animation, frames=len(
            scara.trajectory[0]), interval=2, blit=True, repeat=False)
        # Save Animation
