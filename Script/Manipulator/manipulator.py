...
class Control(object):
    def __init__(self, robot_name, rDH_param, ax_wr):
        ...
        self.plt = plt
        self.figure = self.plt.figure(num=None, figsize=(
            25, 17.5), dpi=80, facecolor='w', edgecolor='k')
        # Resulting trajectory
        self.trajectory = [[], [], []]
        self.plt.gca().add_patch(
            patches.Rectangle((-0.05, -0.05), 0.1, 0.1, label=r'Base', facecolor=[1, 0, 0, 0.25]
                              )
        )
        # Visible -> Joint 1
        self.plt.plot(0.0, 0.0,
                      label=r'Joint 1: $\theta_1 (' + str(self.ax_wr[0] * (
                          180/np.pi)) + ',' + str(self.ax_wr[1] * (180/np.pi)) + ')$',
    ...
    def __fast_calc_fk(self):
        ...
        self.p[0] = round(self.rDH_param.a[0]*np.cos(self.rDH_param.theta[0]) + self.rDH_param.a[1]
                          * np.cos(self.rDH_param.theta[0] + self.rDH_param.theta[1]), self.__rounding_index)
        self.p[1] = round(self.rDH_param.a[0]*np.sin(self.rDH_param.theta[0]) + self.rDH_param.a[1]
                          * np.sin(self.rDH_param.theta[0] + self.rDH_param.theta[1]), self.__rounding_index)
    ...
    def __dh_calc_fk(self, index):
        ...
        Ai_aux[0, 1] = (-1)*(np.sin(self.rDH_param.theta[index])
                             )*np.cos(self.rDH_param.alpha[index])
        Ai_aux[0, 2] = np.sin(self.rDH_param.theta[index]) * \
            np.sin(self.rDH_param.alpha[index])
        # Translation Part
        Ai_aux[0, 3] = self.rDH_param.a[index] * \
            np.cos(self.rDH_param.theta[index])

        # << Calulation Second Row >>
        # Rotational Part
        Ai_aux[1, 0] = np.sin(self.rDH_param.theta[index])
        Ai_aux[1, 1] = np.cos(self.rDH_param.theta[index]) * \
            np.cos(self.rDH_param.alpha[index])
        Ai_aux[1, 2] = (-1)*(np.cos(self.rDH_param.theta[index])
                             )*(np.sin(self.rDH_param.alpha[index]))
        # Translation Part
        Ai_aux[1, 3] = self.rDH_param.a[index] * \
            np.sin(self.rDH_param.theta[index])
    ...
    def forward_kinematics(self, calc_type, theta, angle_repr):
        ...
        if angle_repr == 'deg':
            self.rDH_param.theta = [x * (np.pi/180)
                                    for x in self.__theta_target]
        elif angle_repr == 'rad':
    ...
    def inverse_kinematics(self, p, cfg):
        ...
        cosT_beta_numerator = ((self.rDH_param.a[0]**2) + (
            self.__p_target[0]**2 + self.__p_target[1]**2) - (self.rDH_param.a[1]**2))
        cosT_beta_denumerator = (
            2*self.rDH_param.a[0]*np.sqrt(self.__p_target[0]**2 + self.__p_target[1]**2))
        ...
        if cosT_beta_numerator/cosT_beta_denumerator > 1:
            ...
            if cfg == 0:
                theta_aux[0] = np.arctan2(
                    self.__p_target[1], self.__p_target[0]) - np.arccos(cosT_beta_numerator/cosT_beta_denumerator)
            elif cfg == 1:
                theta_aux[0] = np.arctan2(
                    self.__p_target[1], self.__p_target[0]) + np.arccos(cosT_beta_numerator/cosT_beta_denumerator)

        # Cosine Theorem [Alha]: eq (2)
        cosT_alpha_numerator = (self.rDH_param.a[0]**2) + (
            self.rDH_param.a[1]**2) - (self.__p_target[0]**2 + self.__p_target[1]**2)
        ...
        if cosT_alpha_numerator/cosT_alpha_denumerator > 1:
            ...
            if cfg == 0:
                theta_aux[1] = np.pi - \
                    np.arccos(cosT_alpha_numerator/cosT_alpha_denumerator)
            elif cfg == 1:
    ...
    def inverse_kinematics_jacobian(self, p_target, theta, accuracy, num_of_iter):
        ...
        for i in range(num_of_iter):
            ...
            self.forward_kinematics(
                0, [theta_actual[0], theta_actual[1]], False)

            if np.linalg.norm(self.__p_target - self.p) < accuracy:
                print('[GM] Result found in iteration no. ', i)
                print('[GM] Target Position (End-Effector):')
                print('[GM] p_t  = [x: %f, y: %f]' %
                      (self.__p_target[0], self.__p_target[1]))
                print('[GM] Actual Position (End-Effector):')
                print('[GM] p_ee = [x: %f, y: %f]' % (self.p[0], self.p[1]))
                print('[GM] IK Jacobian (Accuracy Error):')
                print('[GM] Euclidean Distance: %f' %
                      np.linalg.norm(self.__p_target - self.p))
                break
    ...
    def __calc_jacobian_matrix(self, theta):
        ...
        self.jacobian_matrix[0, 0] = round(((-1)*self.rDH_param.a[0]*np.sin(theta[0])) + (
            (-1)*self.rDH_param.a[1]*np.sin(theta[0] + theta[1])), self.__rounding_index)
        self.jacobian_matrix[0, 1] = round(
            (-1)*self.rDH_param.a[1]*np.sin(theta[0] + theta[1]), self.__rounding_index)
        self.jacobian_matrix[1, 0] = round((self.rDH_param.a[0]*np.cos(theta[0])) + (
            self.rDH_param.a[1]*np.cos(theta[0] + theta[1])), self.__rounding_index)
        self.jacobian_matrix[1, 1] = round(
            (self.rDH_param.a[1]*np.cos(theta[0] + theta[1])), self.__rounding_index)
    ...
    def generate_trajectory(self, trajectory_str):
        ...
        self.__p_target = [trajectory_str['target_p']
                           [0], trajectory_str['target_p'][1]]
        self.__theta_target = self.inverse_kinematics(
            self.__p_target, trajectory_str['cfg'])
        ...
        if trajectory_str['interpolation'] == 'linear':
            ...
            x = (1 - time) * trajectory_str['start_p'][0] + \
                time * trajectory_str['target_p'][0]
            y = (1 - time) * trajectory_str['start_p'][1] + \
                time * trajectory_str['target_p'][1]
            ...
            self.inverse_kinematics(
                trajectory_str['start_p'], trajectory_str['cfg'])
            start_theta = self.rDH_param.theta

            self.inverse_kinematics(
                trajectory_str['target_p'], trajectory_str['cfg'])
            target_theta = self.rDH_param.theta

            start_theta_dt = np.linspace(
                start_theta[0], target_theta[0], trajectory_str['step'])
            target_theta_dt = np.linspace(
                start_theta[1], target_theta[1], trajectory_str['step'])

            for i in range(len(start_theta_dt)):
                self.forward_kinematics(
                    1, [start_theta_dt[i], target_theta_dt[i]], 'rad')
                x.append(self.p[0])
                y.append(self.p[1])

        # Display the trajectory results with start and end point
        self.plt.plot(x[0], y[0], marker='o', ms=15, mfc=[
                      1, 1, 0], markeredgecolor=[0, 0, 0], mew=5)
        self.plt.plot(x[len(x) - 1], y[len(y) - 1], marker='o',
                      ms=15, mfc=[1, 1, 0], markeredgecolor=[0, 0, 0], mew=5)
    ...
    def smooth_trajectory(self, trajectory_str):
        ...
        try:
            ...
            if len(trajectory_str) == 2:
                ...
                x = ((1 - time)**2) * trajectory_str[0]['start_p'][0] + 2 * (
                    1 - time) * time * trajectory_str[0]['target_p'][0] + (time**2) * trajectory_str[1]['target_p'][0]
                y = ((1 - time)**2) * trajectory_str[0]['start_p'][1] + 2 * (
                    1 - time) * time * trajectory_str[0]['target_p'][1] + (time**2) * trajectory_str[1]['target_p'][1]

                cfg.append([trajectory_str[0]['cfg']]
                           * trajectory_str[0]['step'])
                cfg.append([trajectory_str[1]['cfg']]
                           * trajectory_str[1]['step'])
            else:
                cfg = []
                time = np.linspace(
                    0.0, 1.0, trajectory_str[0]['step'] + trajectory_str[1]['step'] + trajectory_str[2]['step'])

                # Cubic Bezier Curve
                # p(t) = ((1 - t)^3)*p_{0} + 3*t*((1 - t)^2)*p_{1} + (3*t^2)*(1 - t)*p_{2} + (t^3) * p_{3}, t ∈ [0, 1]
                x = ((1 - time)**3) * (trajectory_str[0]['start_p'][0]) + (3 * time * (1 - time)**2) * (trajectory_str[0]['target_p'][0]) + 3 * (
                    time**2) * (1 - time) * trajectory_str[1]['target_p'][0] + (time**3) * trajectory_str[2]['target_p'][0]
                y = ((1 - time)**3) * (trajectory_str[0]['start_p'][1]) + (3 * time * (1 - time)**2) * (trajectory_str[0]['target_p'][1]) + 3 * (
                    time**2) * (1 - time) * trajectory_str[1]['target_p'][1] + (time**3) * trajectory_str[2]['target_p'][1]

                cfg.append([trajectory_str[0]['cfg']]
                           * trajectory_str[0]['step'])
                cfg.append([trajectory_str[1]['cfg']]
                           * trajectory_str[1]['step'])
                cfg.append([trajectory_str[2]['cfg']]
            ...
            print('[GM] Insufficient number of entry points.')
            print(
                '[GM] The number of entry points must be 3 (Quadratic Curve) or 4 (Cubic Curve).')
    ...
    def check_target_err(self, kinematics_str):
        ...
        if kinematics_str['calc_type'] == 'IK':
            self.inverse_kinematics(
                [kinematics_str['p'][0], kinematics_str['p'][1]], kinematics_str['cfg'])

            if (self.theta[0] < self.ax_wr[0] or self.theta[0] > self.ax_wr[1]) or (self.theta[1] < self.ax_wr[2] or self.theta[1] > self.ax_wr[3]):
                print(
                    '[GM] Calculation Error [The target position (Joint representation) is outside of the working range].')
                print('[GM] Calculation Error [Working range: Theta_1 (%f, %f), Theta_2(%f, %f)]:' % (
                    self.ax_wr[0] * (180/np.pi), self.ax_wr[1]*(180/np.pi), self.ax_wr[2]*(180/np.pi), self.ax_wr[3]*(180/np.pi)))
                print('[GM] Calculation Error [Target Position: Joint]:',
                      self.theta[0]*(180/np.pi), self.theta[1]*(180/np.pi))

                # Mark error points with -> [*].
                self.plt.plot(kinematics_str['p'][0], kinematics_str['p'][1], marker='*', ms=10, mfc=[
                              1, 0, 0], markeredgecolor=[0, 0, 0], mew=2.5)

                return True
            else:
                if (round(self.__p_target[0], 3) != round(self.p[0], 3)) and (round(self.__p_target[1], 3) != round(self.p[1], 3)):
                    print(
                        '[GM] Calculation Error [The target position (Cartesian representation) is outside of the workspace].')
                    print('[GM] Calculation Error [Actual Position: End-Effector]:',
                          round(self.p[0], 3), round(self.p[1], 3))
                    print('[GM] Calculation Error [Target Position: End-Effector]:',
                          round(self.__p_target[0], 3), round(self.__p_target[1], 3))
            ...
            if (theta_aux[0] < self.ax_wr[0] or theta_aux[0] > self.ax_wr[1]) or (theta_aux[1] < self.ax_wr[2] or theta_aux[1] > self.ax_wr[3]):
                ...
                print('[GM] Calculation Error [Working range: Theta_1 (%f, %f), Theta_2(%f, %f)]:' % (
                    self.ax_wr[0] * (180/np.pi), self.ax_wr[1]*(180/np.pi), self.ax_wr[2]*(180/np.pi), self.ax_wr[3]*(180/np.pi)))
                print('[GM] Calculation Error [Target Position: Joint]:',
                      theta_aux[0]*(180/np.pi), theta_aux[1]*(180/np.pi))
    ...
    def __generate_workspace_curve(self, limit, fk_param, increment):
        ...
        for i in range(int(limit[0] * (180/np.pi)), int(limit[1] * (180/np.pi)), increment):
            if fk_param[0] == 0:
                self.forward_kinematics(
                    1, [fk_param[1], i * (np.pi/180)], 'rad')
            elif fk_param[0] == 1:
                self.forward_kinematics(
                    1, [i * (np.pi/180), fk_param[1]], 'rad')
    ...
    def __display_workspace(self, display_type=0):
        ...
        if display_type == 0:
            ...
            # Inner Circle -> Part 1
            x, y = self.__generate_workspace_curve(
                [self.ax_wr[1], self.ax_wr[0]], [1, self.ax_wr[2]], -1)
            x_pN.append(x)
            y_pN.append(y)
            self.plt.plot(x_pN[0], y_pN[0], '-', c=[0, 0, 1, 0.5], linewidth=5)
            # Inner Circle -> Part 2
            x, y = self.__generate_workspace_curve(
                [self.ax_wr[1], self.ax_wr[0]], [1, self.ax_wr[3]], -1)
            x_pN.append(x)
            y_pN.append(y)
            self.plt.plot(x_pN[1], y_pN[1], '-', c=[0, 0, 1, 0.5], linewidth=5)
            # Outer Curve -> Part 1
            x, y = self.__generate_workspace_curve(
                [self.ax_wr[3], 0.0], [0, self.ax_wr[1]], -1)
            x_pN.append(x)
            y_pN.append(y)
            self.plt.plot(x_pN[2], y_pN[2], '-', c=[0, 0, 1, 0.5], linewidth=5)
            # Outer Circle
            x, y = self.__generate_workspace_curve(
                [self.ax_wr[1], self.ax_wr[0]], [1, 0.0], -1)
            x_pN.append(x)
            y_pN.append(y)
            self.plt.plot(x_pN[3], y_pN[3], '-', c=[0, 0, 1, 0.5], linewidth=5)
            # Outer Curve -> Part 2
            x, y = self.__generate_workspace_curve(
                [0.0, self.ax_wr[2]], [0, self.ax_wr[0]], -1)
            x_pN.append(x)
            y_pN.append(y)
            self.plt.plot(x_pN[4], y_pN[4], '-', c=[0, 0, 1, 0.5], linewidth=5)

            self.plt.plot(x_p[0][0], y_p[0][0], '.',
                          label=u"Work Envelop", c=[0, 0, 1, 0.5])

        elif display_type == 1:
            self.plt.plot(x_p, y_p, 'o', c=[0, 0, 1, 0.1])
            self.plt.plot(x_p[0][0], y_p[0][0], '.',
                          label=u"Work Envelop", c=[0, 0, 1, 0.5])
    ...
    def __animation_data_generation(self):
        ...
        self.__animation_dMat = np.zeros(
            (len(self.trajectory[0]), 4), dtype=np.float32)

        for i in range(len(self.trajectory[0])):
            self.inverse_kinematics(
                [self.trajectory[0][i], self.trajectory[1][i]], self.trajectory[2][i])
            self.__animation_dMat[i][0] = self.rDH_param.a[0] * \
                np.cos(self.rDH_param.theta[0])
            self.__animation_dMat[i][1] = self.rDH_param.a[0] * \
                np.sin(self.rDH_param.theta[0])
    ...
    def init_animation(self):
        ...
        self.__line[1].set_data([self.__animation_dMat[0][0], self.__animation_dMat[0][2]], [
    ...
    def start_animation(self, i):
        ...
        self.__line[1].set_data([self.__animation_dMat[i][0], self.__animation_dMat[i][2]], [
                                self.__animation_dMat[i][1], self.__animation_dMat[i][3]])
    ...
    def display_environment(self, work_envelope=[False, 0]):
        ...
        if len(self.trajectory[0]) > 0:
            self.plt.plot(self.trajectory[0][0], self.trajectory[1][0], label=r'Trajectory Points: $p_{(x, y)}$', marker='o', ms=15, mfc=[
                          1, 1, 0], markeredgecolor=[0, 0, 0], mew=5)
            self.plt.plot(self.trajectory[0][0], self.trajectory[1][0], label=r'Initial Position: $p_{(x, y)}$', marker='o', ms=30, mfc=[
                          0, 0.5, 1], markeredgecolor=[0, 0, 0], mew=5)
            self.plt.plot(self.trajectory[0][len(self.trajectory[0]) - 1], self.trajectory[1][len(self.trajectory[1]) - 1],
                          label=r'Target Position: $p_{(x, y)}$', marker='o', ms=30, mfc=[0, 1, 0], markeredgecolor=[0, 0, 0], mew=5)

            self.p = [self.trajectory[0][len(
                self.trajectory[0]) - 1], self.trajectory[1][len(self.trajectory[1]) - 1]]
            self.inverse_kinematics(
                self.p, self.trajectory[2][len(self.trajectory[0]) - 1])

        # Display FK/IK calculation results (depens on type of options)
        self.__display_result()

        # Set minimum / maximum environment limits
        self.plt.axis([(-1)*(self.rDH_param.a[0] + self.rDH_param.a[1]) - 0.2, (1)*(self.rDH_param.a[0] + self.rDH_param.a[1]) +
                      0.2, (-1)*(self.rDH_param.a[0] + self.rDH_param.a[1]) - 0.2, (1)*(self.rDH_param.a[0] + self.rDH_param.a[1]) + 0.2])
    ...
    def __display_rDHp(self):
        ...
        print('[GM] The Denavit-Hartenberg modified parameters of robot %s:' %
              (self.robot_name))
        print('[GM] theta = [%f, %f]' %
              (self.rDH_param.theta[0], self.rDH_param.theta[1]))
        print('[GM] a     = [%f, %f]' %
              (self.rDH_param.a[0], self.rDH_param.a[1]))
        print('[GM] d     = [%f, %f]' %
              (self.rDH_param.d[0], self.rDH_param.d[1]))
        print('[GM] alpha = [%f, %f]' %
              (self.rDH_param.alpha[0], self.rDH_param.alpha[1]))
    ...
    def __display_result(self):
        ...
        print('[GM] Result of Kinematics calculation:')
        print('[GM] Robot: %s' % (self.robot_name))
        ...
        if not (self.__p_target is None):
            ...
            print('[GM] p_t  = [x: %f, y: %f]' %
                  (self.__p_target[0], self.__p_target[1]))
            print('[GM] Target Position (Joint):')
            print('[GM] Theta  = [Theta_1: %f, Theta_2: %f]' %
                  (self.__theta_target[0], self.__theta_target[1]))
            # ACTUAL
            print('[GM] Actual Position (End-Effector):')
            print('[GM] p_ee = [x: %f, y: %f]' % (self.p[0], self.p[1]))
            print('[GM] Actual Position (Joint):')
            print('[GM] Theta  = [Theta_1: %f, Theta_2: %f]' %
                  (self.theta[0], self.theta[1]))
        else:
            print('[GM] Target Position (Joint):')
            print('[GM] Theta  = [Theta_1: %f, Theta_1: %f]' %
                  (self.__theta_target[0], self.__theta_target[1]))
            print('[GM] Actual Position (End-Effector):')
            print('[GM] p_ee = [x: %f, y: %f]' % (self.p[0], self.p[1]))
